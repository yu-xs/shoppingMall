<template>
    <div class="recommend" v-if="dataList.length > 0">
        ...
        <ul class="gridList" v-if="dataList.length > 0">
            <li v-for="i in dataList[10].body.items" @click="goPageView(i.action.path, i.action.type)">
                <img :src="`${i.img_url}`" />
                <p class="title">{{ i.product_name }}</p>
            </li>
        </ul>
        ...
        <ul class="gridList" v-if="dataList.length > 0">
            <li v-for="i in dataList[12].body.items" @click="goPageView(i.action.path, i.action.type)">
                <img :src="`${i.img_url}`" />
                <p class="title">{{ i.product_name }}</p>
            </li>
        </ul>
        ...
        <ul class="gridList" v-if="dataList.length > 0">
            <li v-for="i in dataList[14].body.items" @click="goPageView(i.action.path, i.action.type)">
                <img :src="`${i.img_url}`" />
                <p class="title">{{ i.product_name }}</p>
            </li>
        </ul>
        ...
        <ul class="gridList" v-if="dataList.length > 0">
            <li v-for="i in dataList[16].body.items" @click="goPageView(i.action.path, i.action.type)">
                <img :src="`${i.img_url}`" />
                <p class="title">{{ i.product_name }}</p>
            </li>
        </ul>
...
async function getDataList() {
    console.log('请求数据')
    // 分类数据
    const data = await axios.post(`/api/v1/home/category_v2?cat_id=1053`);
